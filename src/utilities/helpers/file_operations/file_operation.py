...
class File():
        ...
        try:
            ...
            if os.path.exists(filepath):
                ...
                with open(filepath, 'r') as file:   # 
                    ...
                    try:
                        #content = file.read() # re.sub(r"[\n\t\s]*", "", str(file.read()))    # remove all formatting and whitespace
                        ...
                        if mode == 'd':
                            content = dict(json.loads(re.sub(r"[\n\t\s+]", " ", file.read())))
                                ...
                                #content = json.dumps(re.sub(r"[\n\t\s]*", "", str(file.read())), indent=None, separators=(',', ':'))
                                #content = json.dumps(file.read(), indent=None, separators=(',', ':'))
                                content = re.sub(r"[\n\t\s]*", "", str(file.read()))
    ...
    #          returnAsUnformattedJson:bool = True,
    ...
    #             with open(filepath, 'r') as file:   # 
    #                 if returnAsUnformattedJson is True:
    #                     content = json.dumps(re.sub(r"[\n\t\s]*", "", file.read()))  
    #                 elif removeFormat is True:
    #                     content = re.sub(r"[\n\t\s]*", "", file.read())
    #                 else:
    #                     content = file.read()
